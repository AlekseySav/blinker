...
int main(int argc, char* argv[])
{
	...
	shader.SetUniform("u_LightPosition", glm::vec3(0.0f, 100.0f, 20.0f));
...
#define UPD_MS	1000 / UPS
	...
	while (!window.IsClosed()) {
		if (updates.SynchronizedPassed(UPD_MS)) {
			...
			shader.SetUniform("u_View", camera.ViewMatrix);
			shader.SetUniform("u_LightPosition", camera.Position);
		...
		for (float z = zz - 450.0f; z < zz + 100.0f; z += 40.0f)
			for (float x = xx - 300.0f; x < xx + 300.0f; x += 40.0f)
				block.OnRender(renderer, glm::vec3(x, 0.0f, z));
		window.SwapBuffers();
